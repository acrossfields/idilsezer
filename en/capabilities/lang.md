---
category: cap_entry
lang: en
cap_id: 1
title: Languages
---
<table class="table-caplang" style="margin: 5px 0 20px 0;">
  <tr>
    <td>
      <b>Turkish</b>
    </td>
    <td>
      Native speaker
    </td>
  </tr>
</table>

<table class="table-caplang" style="margin: 0 0 8px 0;">
  <tr>
    <td>
      <b>English</b>
    </td>
    <td>
      Advanced proficiency
    </td>
  </tr>
</table>

<div class="cap-indented" style="margin-bottom: 20px;">
  <p style="margin: 0; padding: 4px 0 16px 0;">
    The timeline below illustrates when and in which areas I developed my English skills.
  </p>
  <img src="/idilsezer/assets/images/en_timeline.png" alt="EN Timeline" style="width: 100%; height: auto;">
  <table class="table-caplangsub" style="padding: 15px 0 10px 0;">
    <tr>
      <td>
        <b>IELTS Academic</b>
      </td>
      <td>
        <p style="font-size: 14px;">
          <b>English proficiency test</b>
        </p>
        <p style="margin-bottom: 5px; font-size: 14px;">
          <i>by British Council, IDP, and Cambridge English</i>
        </p>
        <hr style="border: none; border-top: 1px solid #898895; margin-bottom: 5px;">
        <p style="margin-bottom: 5px; font-size: 14px;">
          Overall score: 8.5 / 9.0
        </p>
        <p style="margin-bottom: 5px; font-size: 14px;">
          Listening: 8.5 • Reading: 9.0 • Writing: 8.0 • Speaking: 8.5
        </p>
        <hr style="border: none; border-top: 1px solid #898895; margin-bottom: 5px;">
        <p style="font-size: 12px;">
          2014 - Istanbul, Türkiye
        </p>
      </td>
    </tr>
  </table>
</div>

<table class="table-caplang" style="margin: 0 0 8px 0;">
  <tr>
    <td>
      <b>Portuguese (Brazil)</b>
    </td>
    <td>
      Intermediate proficiency
    </td>
  </tr>
</table>

<div class="cap-indented" style="margin-bottom: 30px;">
  <p style="margin: 0; padding: 4px 0 12px 0;">
    I acquired my Brazilian Portuguese language skills through language immersion while living in São Paulo and have communicated in it in everyday and social contexts. I have not used it in a professional capacity.
  </p>
  <table class="table-caplangsub" style="padding: 0 0 10px 0;">
    <tr>
      <td>
        <b>CELPE-Bras</b>
      </td>
      <td>
        <p style="font-size: 14px;">
          <b>Brazilian Portuguese proficiency exam & certificate</b>
        </p>
        <p style="margin-bottom: 5px; font-size: 14px;">
          <i>by the Brazilian Ministry of Education</i>
        </p>
        <p style="margin-bottom: 5px; font-size: 14px;">
          "Certificado de Proficiência em Língua Portuguesa para Estrangeiros (Certificate of Proficiency in Portuguese for Foreigners)"
        </p>
        <hr style="border: none; border-top: 1px solid #898895; margin-bottom: 5px;">
        <p style="margin-bottom: 5px; font-size: 14px;">
          Level: Intermediate
        </p>
        <hr style="border: none; border-top: 1px solid #898895; margin-bottom: 5px;">
        <p style="font-size: 12px;">
          2018 - São Paulo, Brazil
        </p>
      </td>
    </tr>
  </table>
</div>
